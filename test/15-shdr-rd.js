...
const {stringifyConst,SHT,STT,STV,STB,SHN,DT} = require('../lib/constants'),
                 {expect} = require('chai'),
...
const TEST_PROG_32 = '/usr/lib32/ld-2.31.so';
...
const tableRE32 = '(?:\\[(?:\\s|\\d){1,3}\\]) (?<sh_name>.{17}) (?<sh_type>\\w{1,})(?:\\s{1,})(?<sh_addr>\\d|\\w{8})\\s{1,}(?<sh_offset>\\d|\\w{6})\\s{1,}(?<sh_size>\\d|\\w{6})\\s{1,}(?<sh_entsize>\\d{2})\\s{1,}';
...
const symbolRE32='(?:\\d{1,}:) (?<st_value>(?:\\w|\\d){8})\\s{1,}(?<st_size>\\d{1,})\\s{1,}(?<st_type>\\w{1,})\\s{1,}(?<st_bind>\\w{1,})\\s{1,}(?<st_other>\\w{1,})\\s{1,}(?<st_shndx>\\d|\\w{1,})\\s{0,}(?<name>(?:\\w|\\@|\\.|\\d){1,}(?: \\(\\d\\)){0,1}){0,1}\\s{0,}\\n';

const tagsRE='(?<d_tag>0x(?:\\d|\\w){16})\\s{1,}\\((?<type>\\w{1,})\\)\\s{1,}(?<d_un>(?:\\d|\\w).*)';
const tagsRE32='(?<d_tag>0x(?:\\d|\\w){8})\\s{1,}\\((?<type>\\w{1,})\\)\\s{1,}(?<d_un>(?:\\d|\\w).*)';

describe('SHdr reading', function() {
  ...
  describe('32 bit', function() {
    before(async function() {
      ...
      readelf = execSync('/bin/readelf -S '+fpath).toString();
      readelfTable = [];
      readelf.match(new RegExp(tableRE32,'g')).forEach((row)=>{
        readelfTable.push(row.match(new RegExp(tableRE32)));
      });
    });
    describe('SHdrTable.buildFromHandle()', function() {
      let fhandle,ehdr,table;
    ...
    describe('SymTab', function() {
      ...
      before(function() {
        ...
        readelf = execSync('/bin/readelf -s '+fpath).toString();
        readelfTable = [];
        readelf.match(new RegExp(symbolRE32,'g')).forEach((row)=>{
          readelfTable.push(row.match(new RegExp(symbolRE32)));
        });
      });
      it('builds SymTab from .dynsym section', function() {
        // tests before
        ...
        tab.forEach((sym,i)=>{
          ...
          ['st_value','st_size','st_shndx'].forEach((prop,i)=>{
            ...
            if(typeof sym[prop] == 'bigint')
              ...
              expect(sym[prop],prop+' '+chk[prop]).equal(num);
          });
          let st_type = stringifyConst(sym.st_type,STT).replace('STT_','');
          expect(st_type).match(new RegExp(chk.st_type,'i'));
          let st_bind = stringifyConst(sym.st_bind,STB).replace('STB_','');
    ...
    describe('DynTags', function() {
      ...
      before(function() {
        ...
        readelf = execSync('/bin/readelf -d '+fpath).toString();
        readelfTable = [];
        readelf.match(new RegExp(tagsRE32,'g')).forEach((row)=>{
          readelfTable.push(row.match(new RegExp(tagsRE32)));
        });
      });
      it('builds SymTab from .dynsym section', function() {
        // tests before
