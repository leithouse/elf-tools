...
const tableRE32 = '(\\w*)\\s{1,}((?:\\w|\\d){8})\\s{1,}((?:\\w|\\d){10})\\s{1,}((?:\\w|\\d){10})\\s{1,}((?:\\w|\\d){7})\\s{1,}((?:\\w|\\d){7})\\s{1,}(.{3})\\s{1,}(0x(?:\\d|\\w){1,})';
    ...
    describe('PHdrTable.buildFromHandle()', function() {
      ...
      it('header data matches that reported by readelf -l', function() {
        table.forEach((hdr,i)=>{
          let chk = [];
          chk.push(stringifyConst(hdr.p_type,PT).replace('PT_',''));
          ['p_offset','p_vaddr','p_paddr','p_filesz','p_memsz'].forEach((key,i)=>{
            let pad = i==0?6:(i==1||i==2)?8:5;
            chk.push('0x'+hdr[key].toString(16).padStart(pad,'0'));
          });
          let flags = '';
          if((hdr.p_flags&4)==4)
            flags += 'R';
