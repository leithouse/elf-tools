...
class SHdr {
  ...
  rebuildData() {
    ...
    let subs = ['strTab','symTab','relTab','dynTags','notes','options'];
    let iMax = subs.length;
    for(let i=0;i<iMax;i++) {
      if(this[subs[i]]) {
        this.data = this[subs[i]].buffer;
        if(i==1) { // set sh_info for reloc section
          let jMax = this.symTab.length;
          for(let j=0;j<jMax;j++) {
            if(this.symTab[j].st_bind != STB.LOCAL) {
              this.sh_info = j;
              break;
            }
          }
    ...
    if(this.sh_size < this.sh_entsize)
      this.sh_size = this.sh_entsize;
    if(this.N == 64)
  ...
  recompute(pos) {
    ...
    this.sh_offset = this.sh_addr = pos;
