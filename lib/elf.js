...
class Elf {
  ...
  addSection(opt) {
    ...
    if(segment) {
      if(segment == 'load')
        segment = this.phdr.find(x=>x.p_type==constants.PT.LOAD);
   ...
   */
  addInterp(interp) {
    let shdr = this.shdr.addSection({
      sh_type:constants.SHT.PROGBITS,
      name:'.interp',
      data:Buffer.from(interp+'\0')
    });
    this.phdr.addSegment({
      p_type:constants.PT.INTERP,
      parentSegment:this.phdr.find(x=>x.p_type==constants.PT.LOAD),
      shdr
    });
  }
  /**
   ...
   * @param [fpath] {String} Defaults to this.jsonPath
   * @async
   */
  async loadArchHeaderJson(fpath) {
    if(!fpath)
      fpath = this.jsonPath;
    else if(!/\/|(\.json$)/.test(fpath))
      fpath = path.join(__dirname,'headers',fpath+'.json');
    if(!fpath)
      throw new Error('No JSON file path');
    let json = JSON.parse(await fsp.readFile(fpath,'utf8'));
    this.archHeader = json;
    ['DT','PT','SHT','NT'].forEach((def)=>{
      if(!json[def])
        return;
      for(let prop in json[def])
        constants[def][prop] = json[def][prop];
    });
    return json;
  }
}
