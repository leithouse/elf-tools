...
class EHdr {
  ...
  constructor(opt) {
    ...
    this.structProps.forEach((prop)=>{
      ...
      else if(prop.key == 'e_phentsize')
        this.e_phentsize = this.N==64? 54 /* (5*4+3*8) */: 32 /* 8*4 */;
      else if(prop.key == 'e_shentsize')
        this.e_shentsize = this.N==64? 64 /* (4*4+6*8) */: 40 /* 10*4 */;
      else
   ...
   * @readonly
   */
  get N() { return this.e_ident.N }
  /**
   * @type {Boolean}
   * @readonly
   */
  get LE() { return this.e_ident.LE }
  /**
   * @type {StructProp[]}
   * @readonly
   */
