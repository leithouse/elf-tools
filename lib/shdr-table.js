...
class SHdrTable extends Array {
  ...
  recompute(pos) {
    ...
    for(let i=0;i<this.e_shnum;i++) {
      ...
      if(shdr.symTab) {
        let lastLocal=0;
        shdr.symTab.forEach((sym,i)=>{
          if(sym.st_bind == STB.LOCAL)
            lastLocal=i;
        });
        shdr.sh_info = lastLocal+1;
      }
   ...
   * @param [opt.altSHT] {Integer} Use differnt SHT idefentifier than SHT_NOTE
   * @returns {SHdr}
   */
  addNotesSection(opt) {
    let name = opt.name;
    ...
    if(opt.sh_flags === undefined)
      opt.sh_flags = 0x2;
    opt.sh_type = opt.altSHT || SHT.NOTE;
    opt.sh_addralign = 0x4;
    if(!opt.data)
